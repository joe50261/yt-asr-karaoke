...
- 字幕資料自帶的 `\n` 行標記——json3 的獨立 `\n` seg，或內嵌在字裡的
  `\n`（`"word\n"`、`"\nword"` 同樣視為行標記）；
- 說話者變更（`>>` 開頭的字）；
- 整軌沒有任何 `\n` 標記時，退回**事件邊界**：每個非 `aAppend` 的新事件
  斷行（asr 的行結構本來就落在事件上，`\n` 只是它的顯示標記；`aAppend`
  是 roll-up 的續行，不斷）。時間間隔仍是併行後盾：前後字間隔超過
  `LINE_BREAK_GAP_MS`（700 ms）也斷。有 `\n` 結構時兩者都不適用——事件
  是否合併聽資料的（翻譯軌跨事件併行是 YouTube 有意的）；
- 安全閥：組出來的行若字起點跨度超過 `LINE_MAX_SPAN_MS`（12 s，正常 asr
  行只有 2–7 s），代表上面所有結構都缺席（例如整片塞在單一事件裡），
  按 `LINE_SPLIT_TARGET_MS`（4 s）在字界強制重切。
