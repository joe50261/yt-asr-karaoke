...
每個字隨語音同步亮起，用的是字幕軌**真實的逐字時間戳**（絕不模擬）。也支援
YouTube 的**自動翻譯**字幕，包含**雙語對照（dual-track）**顯示。

## 運作原理（短版）

擴充功能是對「播放器當前顯示的自動字幕」的**被動綁定**——它自己從不抓字幕：
...
讓播放器自己的字幕顯示——絕不覆蓋別的字幕、也絕不留下空白字幕。
...
- **原生播放** —— 切換卡拉OK的*繪製方式*。關（預設）是上述的**覆蓋層**模式
  ...
  **雙語對照**照樣可用（兩個堆疊的原生視窗）。覆蓋層專屬的**字幕樣式**預設
  在原生模式不適用——原生開啟時選單會**停用**（但絕不清空）該下拉。
  **字幕全文**按鈕移到我們右上欄的第三格（Karaoke 與 ⚙ 之下——原生模式沒有
  覆蓋框可以停靠，而左上角屬於 YouTube 自己的全螢幕標題/付費宣傳區）。
  當場切換模式會讓播放器重載字幕、立即生效；當場關掉（或按 Karaoke 開關）
  也用同一方式還原真實字幕——除非你已經自己換了別的軌，那我們絕不覆寫。
  見「架構」——它是獨立的可熱抽換模組（`yk-native.js`：cook 加上模式邊緣），
  外加 `yk-capture.js` 裡薄薄的 transform 接縫；捕獲池永遠保存**原始** body
  （經保存的原生 getter 讀取），逐字稿與雙語 cook 絕不會吃到煮過的內容。
- **字幕樣式** —— 覆蓋層的外觀，即時生效：
  - `預設` —— 金色當前字，柔光暈加輕微放大。
...
- **自動翻譯** —— 自動**驅動**的目標。它與上面的**雙語對照**完全**正交**：
  ...
  然後**站開**（不輪詢、不覆寫）。切到別的語言、或字幕漂走（YouTube 還原
  字幕、廣告）後會重新驅動。切回`關閉`只是**停止自動驅動**、**別的什麼都
  不做**（絕不動**雙語對照**、不塌陷顯示）。因為兩者正交，任何組合都成立
  ...
  `yk-yt.selectAsrVariant` 拒絕播放器沒有提供的語言代碼，絕不捏造。
- **譯文在上** —— 雙語時把譯文疊在原文上方而不是下方。覆蓋層與側欄
  逐字稿都適用（兩者同一列序）。

## 換行

行在**字幕資料自己**斷行處斷（YouTube 的 json3 帶自己的 `\n` 行標記——
來源語意上的行），外加說話者變更（`>>`）。**沒有字數上限**——長行由 CSS
自然折行，不會把句子攔腰截斷。唯一的後備是完全沒有 `\n` 結構的字幕，
改在語音停頓處斷行。

## 它不做的事

- **不**抓字幕（也抓不到——`timedtext` 被 `pot` 擋）。只在你主動開啟
  **自動翻譯**時驅動播放器的字幕選擇，而且只透過播放器自己的 `setOption`
  API（絕無裸 fetch）。
- **不**靜音、不改音訊、不影響自動播放下一支。
- **不**模擬或內插逐字時間——高亮只用字幕資料真實的 `tOffsetMs`。
  有些影片的自動字幕只有行級時間（無逐字 offset），那就逐行高亮。
- **不**覆蓋別的字幕：只在實際顯示 asr 卡拉OK時隱藏原生字幕。
...
（`yk-di.js` 最先、`yk-main.js` 最後）。**全程沒有 `innerHTML`**——
YouTube 強制 Trusted Types，DOM 一律用 `textContent` / `replaceChildren` 建。
...
- `yk-capture.js` —— `fetch`/XHR 攔截器。永遠把**原始** asr timedtext body
  捕進池裡（經保存的原生 getter 讀取），並提供 transform 接縫
  （`registerTransform`/`clearTransform`，page-global），讓原生模式能換掉
  **播放器**收到的 body 而絕不污染池。無 transform 時＝與被動模式位元組等同。
...
- `yk-autodrive.js` —— **唯一的自動選軌 driver**：自動翻譯的 one-shot
  ...
  請求時把當前變體重選一次（設定當場翻動使畫面上的 body 過期時）。
  沒有其他人驅動播放器選軌。重選就是強制重抓（live 實證：任何 `setOption`
  選軌都真發請求——播放器沒有字幕快取）。一個 boolean 旗標，逐 tick 重試到
...
- `yk-native.js` —— **原生播放模式**，獨立的可熱抽換模組：純函數
  ...
  `yk-capture` 的 impure `cook(url, body)` transform，以及一個**極簡**的
  模式邊緣（`syncEdge`/`standDown`/`isOn`——狀態只有一個 `on` 旗標＋一個
  設定簽名）。核心就是「劫持＋切一遍」：transform 煮掉播放器抓的一切
  （而播放器在每次選軌/載片都會抓），所以模式大多自然生效；只有當場翻
  設定（nativeMode、開著時的 dual/top）才請 autodrive 切一遍。
  選軌變更根本不觀測。退場時用一次重選還原真實字幕，絕不覆寫使用者
  自己選的軌。
- `yk-engine.js` —— 生命週期指揮：選軌、綁定、渲染迴圈、SPA 重啟、開關。
  tick 有導航守門（URL 已換 → 什麼都不驅動，等正式 teardown）。原生模式下
  它只每 tick 呼叫 `native.syncEdge()`、在進入邊緣交接字幕區域，並在
  Karaoke-OFF 點擊時給 teardown 帶 `restoreCaption`（導航/熱抽換的 teardown
  不還原——下一支影片/新實例自己會重煮）。
...
注入的依賴交談、從不經全域，且每個都小到能獨立閱讀/替換。持有 DOM、
